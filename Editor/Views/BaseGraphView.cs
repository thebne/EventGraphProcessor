...
namespace GraphProcessor
{
	...
	public class BaseGraphView : GraphView, IDisposable
	{
		...
		public event Action onGraphPropertyUpdate;
		public event Action onAfterGraphChanged;
		...
		void GraphChangesCallback(GraphChanges changes)
		{
			...
			schedule.Execute(() => onAfterGraphChanged?.Invoke()).ExecuteLater(1);
		}
		...
		protected virtual void KeyDownCallback(KeyDownEvent e)
		{
			...
			if (!e.altKey && !e.commandKey && !e.ctrlKey && e.character != '\0')
			{
				currentKeyPress = e.character;
			}
		}
		...
		void MouseDownCallback(MouseDownEvent e)
		{
			...
			if (e.button == 0)
			{
				...
				if (NodeProvider.GetGenericShortcuts().TryGetValue(currentKeyPress, out var nodeType))
				{
					var mousePos = (e.currentTarget as VisualElement).ChangeCoordinatesTo(contentViewContainer, e.localMousePosition);
					var node = BaseNode.CreateFromType(nodeType, mousePos);
					RegisterCompleteObjectUndo($"Added node {node}");
					AddNode(node);
				}
			}
			...
			if (dragData != null)
			{
				...
				if (exposedParameterFieldViews.Any())
				{
					foreach (var paramFieldView in exposedParameterFieldViews)
					{
						...
						var paramNode = BaseNode.CreateFromType< GetParameterNode >(mousePos);
						paramNode.SetParameterGUID(paramFieldView.parameter.guid);
						AddNode(paramNode);
					}
				}
			}
