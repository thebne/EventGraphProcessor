...
namespace GraphProcessor
{
    public class GroupView : UnityEditor.Experimental.GraphView.Group
	{
        ...
        {
            // Fix (OP): Don't remove if reloading the view. The group reference is mutable
            //. and therefore will also change the serializable data - not needed (and buggy) when reloading
            if (owner.reloading)
                return;
