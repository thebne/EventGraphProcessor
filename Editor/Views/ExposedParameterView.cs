...
namespace GraphProcessor
{
	public class ExposedParameterView : PinnedElementView
	{
        ...
        private Dictionary<BaseGraph, ExposedParameterFieldFactory> graphToFactory 
            = new Dictionary<BaseGraph, ExposedParameterFieldFactory>();
        ...
        {
            content.Clear();
            foreach (var graph in graphView.graph.globalGraphsList)
                graphToFactory[graph] = new ExposedParameterFieldFactory(graph, graph.exposedParameters);

            var baseGraphs = new List<BaseGraph> { graphView.graph };
            baseGraphs.AddRange(graphView.graph.globalGraphsList);

            foreach (var graph in baseGraphs)
            {
                var isOwnParam = graph == graphView.graph;

                if (!isOwnParam)
                {
                    var label = new Label($"Globals ({graph.name})");
                    label.AddToClassList("globals-lbl");
                    content.Add(label);
                }
                ...
                foreach (var param in graph.exposedParameters)
                {
                    ...
                    ExposedParameterFieldFactory factory = null;
                    if (!isOwnParam)
                        factory = graphToFactory[graph];

                    var row = new BlackboardRow(
                        new ExposedParameterFieldView(graphView, param, isOwnParam),
                        new ExposedParameterPropertyView(graphView, param, factory)
                    );
