...
namespace GraphProcessor
{
	...
	public abstract class BaseNode
	{
		...
		void InitializeInOutDatas()
		{
			...
			foreach (var method in methods)
			{
				var customPortBehaviorAttributes = method
					.GetCustomAttributes< CustomPortBehaviorAttribute >()
					.ToArray();
				CustomPortBehaviorDelegate behavior = null;

				if (customPortBehaviorAttributes.Length == 0)
					continue ;
				...
				foreach (var attr in customPortBehaviorAttributes)
				{
					if (nodeFields.ContainsKey(attr.fieldName))
						nodeFields[attr.fieldName].behavior = behavior;
					else
						Debug.LogError("Invalid field name for custom port behavior: " + method + ", " + attr.fieldName);
				}
			}
		}
