...
namespace GraphProcessor
{
	...
	public class BaseGraph : ScriptableObject, ISerializationCallbackReceiver
	{
		...
		[SerializeField]
		public List<BaseGraph> globalGraphsList = new List<BaseGraph>();
		...
		public IEnumerable<ExposedParameter> allExposedParameters
			=> exposedParameters.Concat(globalGraphsList.SelectMany(g => g.exposedParameters));
...
#pragma warning restore CS0618
		...
		{
			var param = GetExposedParameterFromGUID(guid);
		...
		private ExposedParameter GetExposedParameterFromPredicate(Predicate<ExposedParameter> predicate)
        {
			var param = exposedParameters.Find(predicate);

			// if didn't find, iterate through parents
			var globalParams = globalGraphsList.GetEnumerator();
			while (param == null && globalParams.MoveNext())
				param = globalParams.Current.GetExposedParameterFromPredicate(predicate);
		...
		public ExposedParameter GetExposedParameter(string name)
			=> GetExposedParameterFromPredicate(e => e?.name == name);
		...
		public ExposedParameter GetExposedParameterFromGUID(string guid) 
			=> GetExposedParameterFromPredicate(e => e?.guid == guid);
		...
		public bool SetParameterValue(string name, object value)
			=> SetParameterValue(GetExposedParameter(name), value);
		public bool SetParameterValueByGUID(string guid, object value)
			=> SetParameterValue(GetExposedParameterFromGUID(guid), value);

		bool SetParameterValue(ExposedParameter e, object value)
		{
			if (e == null)
				return false;
		...
		public object GetParameterValue(string name)
			=> GetExposedParameter(name)?.value;

		public object GetParameterValueByGUID(string guid)
			=> GetExposedParameterFromGUID(guid)?.value;
